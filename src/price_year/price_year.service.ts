...
export class PriceYearService {
  constructor(
    @InjectModel(priceYear) private priceYearRepository: typeof priceYear,
  ) {}

  async create(createPriceYearDto: CreatePriceYearDto) {
    const newpriceYear = await this.priceYearRepository.create(
      createPriceYearDto,
    );
    if (!newpriceYear) {
  ...
  async findAll() {
    const allPriceYear = await this.priceYearRepository.findAll({
      include: { all: true },
    });
    if (!allPriceYear) {
      throw new HttpException(
