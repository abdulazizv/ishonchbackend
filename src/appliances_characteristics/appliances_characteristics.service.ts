...
export class AppliancesCharacteristicsService {
  constructor(
    @InjectModel(Appliances) private appliancesRepository: typeof Appliances,
  ...
  async findAll() {
    const allAppliances = await this.appliancesRepository.findAll({
      include: { all: true },
    });
  ...
  async update(
    ...
    const newAppliances = await this.appliancesRepository.update(
